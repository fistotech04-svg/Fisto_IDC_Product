...
const ColorPicker = ({ color, onChange, onClose, onReset }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100001] flex justify-end items-start pt-[80px] pr-[30px] pointer-events-none">
      <div className="fixed inset-0 pointer-events-auto" onClick={onClose} />

      <div className={`bg-white rounded-[32px] shadow-[0_30px_90px_rgba(0,0,0,0.3)] w-[320px] overflow-hidden flex flex-col transition-all duration-500 ease-in-out transform scale-100 animate-in zoom-in-95 pointer-events-auto relative z-10 ${viewMode === 'advanced' ? 'max-h-[85vh]' : 'max-h-[500px]'}`}>
        {/* Top Header */}
        <div className="flex items-center justify-between px-7 pt-6 pb-2 bg-white sticky top-0 z-20">
          <div className="flex items-center gap-[46px] ml-4">
            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em]">R</span>
            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em]">G</span>
            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em]">B</span>
          </div>
          <div className="flex items-center gap-4">
            <RotateCcw size={16} className="text-gray-400 cursor-pointer hover:text-indigo-600 transition-colors" onClick={onReset} />
            <X size={20} className="text-gray-400 cursor-pointer hover:text-red-500 transition-colors" onClick={onClose} />
          </div>
        </div>

        <div className="px-7 pb-8 pt-2 overflow-y-auto custom-scrollbar flex flex-col">
          {/* Transitioning Advanced Section (Now on top of presets) */}
          <div className={`overflow-hidden transition-all duration-500 ease-in-out ${viewMode === 'advanced' ? 'max-h-[500px] opacity-100 mb-8 mt-2' : 'max-h-0 opacity-0 mb-0 mt-0'}`}>
            <div className="space-y-6">
              {/* Saturation/Value Area */}
              <div
                ref={containerRef}
                onMouseDown={handleMouseDown}
                className="relative w-full h-44 rounded-[22px] cursor-crosshair overflow-hidden shadow-inner border border-gray-100"
                style={{ backgroundColor: `hsl(${hsv.h}, 100%, 50%)` }}
              >
                <div className="absolute inset-0 bg-gradient-to-r from-white to-transparent" />
                <div className="absolute inset-0 bg-gradient-to-t from-black to-transparent" />
                <div
                  className="absolute w-5 h-5 border-[3px] border-white rounded-full shadow-[0_2px_6px_rgba(0,0,0,0.4)] -translate-x-1/2 translate-y-1/2 pointer-events-none"
              ...
              <div className="flex items-center gap-5">
                <div
                  ...
                  className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-500 cursor-pointer hover:bg-indigo-50 hover:text-indigo-600 transition-all shadow-sm active:scale-95"
                >
                  <Pipette size={20} />
                </div>
                <div className="w-8 h-8 rounded-full border border-gray-200 shrink-0 shadow-sm" style={{ backgroundColor: color }} />
              ...
              <div className="grid grid-cols-3 gap-4">
                {['r', 'g', 'b'].map((key) => (
                  <div key={key} className="flex flex-col items-center gap-2">
                    <div className="w-full h-11 border border-gray-200 rounded-xl flex items-center justify-center bg-gray-50/50 focus-within:bg-white focus-within:border-indigo-300 focus-within:ring-4 focus-within:ring-indigo-50 transition-all">
                      <input
                        type="number"
                        min="0" max="255"
                        value={rgb[key]}
                        ...
                        className="w-full text-center text-[16px] font-bold text-gray-800 outline-none bg-transparent"
                      />
                    </div>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-6 gap-3 mb-8">
            {presets.map((p) => (
              <button
                ...
                className={`w-8 h-8 rounded-[10px] border border-gray-100 transition-all active:scale-90 hover:scale-110 shadow-sm ${color.toUpperCase() === p.toUpperCase() ? 'ring-2 ring-indigo-500 ring-offset-2' : ''}`}
          ...
          <div
            ...
            className="flex items-center gap-5 group cursor-pointer hover:bg-gray-50/50 p-2 -mx-2 rounded-2xl transition-colors mb-6"
          >
            <div className={`w-11 h-11 rounded-full rainbow-wheel-icon shadow-lg transform transition-all duration-500 ${viewMode === 'advanced' ? 'rotate-180 scale-90' : 'group-hover:rotate-12 hover:scale-110'}`} />
            <span className="text-[16px] font-bold text-gray-700 group-hover:text-black transition-colors">
        ...
        return (
          <div className="flex flex-col items-center justify-center">
            <div
              ...
              className="border border-gray-300 rounded-[12px] bg-white shadow-sm overflow-hidden mb-1 flex flex-col items-center cursor-pointer hover:bg-gray-50 transition-colors group relative w-[130px] h-[90px]"
            >
              {popupText ? (
                // Template Preview
                <div className="w-full h-full relative overflow-hidden bg-gray-50">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors">
                    <ArrowRightLeft size={24} className="opacity-0 group-hover:opacity-100 text-white transition-opacity bg-black p-1.5 rounded-md shadow-md absolute top-2 right-2" />
                  </div>
                </div>
              ) : (
                // Placeholder
                <div className="flex flex-col items-center justify-center w-full h-full p-2">
                  <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 mb-2">
                    <rect x="4" y="5" width="16" height="4" rx="1" />
                    <rect x="4" y="14" width="7" height="5" rx="1" />
                    <line x1="15" y1="14" x2="20" y2="14" />
                    <line x1="15" y1="18" x2="20" y2="18" />
                  </svg>
                  <span className="text-[11px] text-gray-500 font-medium text-center leading-tight">Choose Template</span>
                </div>
              )}
            </div>
          </div>
      ...
      case 'download':
        ...
        return (
          <div className="flex flex-col items-center gap-1">
            <div
              className="border border-gray-400 border-dashed rounded-[10px] p-1.5 bg-white shadow-sm overflow-hidden mb-1 flex flex-col items-center cursor-pointer hover:border-indigo-400 group relative transition-colors"
              onClick={() => downloadFileInputRef.current?.click()}
              title="Click to replace download image"
            >
              <div className="w-[3.5vw] h-[3.5vw] bg-gray-50 rounded-[6px] flex items-center justify-center overflow-hidden relative">
                {previewSrc ? (
                  <>
                    ...
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <div className="flex flex-col items-center text-white">
                        <Upload size={14} />
                        <span className="text-[9px] font-medium mt-0.5">Replace</span>
                      </div>
                    </div>
                  </>
                ) : (
                  <div className="w-full h-full bg-indigo-50 flex items-center justify-center p-1 group-hover:bg-indigo-100 transition-colors">
                    <div className="flex flex-col items-center justify-center">
                      <FileText size={20} className="text-indigo-400 mb-1" />
                      <span className="text-[8px] text-indigo-600 font-medium opacity-0 group-hover:opacity-100 absolute bottom-1">Upload</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            <span className="text-[10px] text-gray-500 font-medium uppercase tracking-wider max-w-[80px] truncate" title={hasCustom ? "Custom Image Selected" : filename}>
            ...
            {hasCustom && (
              <button
                ...
                className="text-[9px] text-red-500 hover:text-red-700 font-medium flex items-center gap-0.5 mt-0.5"
              >
                <X size={8} /> Reset
              </button>
            )}
      ...
      if (activeElementInfo) {
        ...
        if (isText && TextEditorComponent) {
          return (
            <div className="mt-4 border-t border-gray-200 pt-3">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm font-bold text-gray-900">Text Properties</span>
          ...
          return (
            <div className="mt-4 border-t border-gray-200 pt-3">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm font-bold text-gray-900">Image Properties</span>
                <div className="h-[1px] flex-grow bg-gray-200"></div>
          ...
          return (
            <div className="mt-4 border-t border-gray-200 pt-3">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm font-bold text-gray-900">Gif Properties</span>
                <div className="h-[1px] flex-grow bg-gray-200"></div>
          ...
          return (
            <div className="mt-4 border-t border-gray-200 pt-3">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm font-bold text-gray-900">Video Properties</span>
                <div className="h-[1px] flex-grow bg-gray-200"></div>
        ...
        <div className="p-4 pt-0 animate-fadeIn space-y-4">
          ...
          {interactionType === 'popup' && activePopupElement === 'background' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex items-center gap-3">
                <h3 className="text-sm font-bold text-gray-900 whitespace-nowrap">Background Color</h3>
                <div className="h-[1px] flex-grow bg-gray-200"></div>
              </div>

              {/* Fill Row */}
              <div className="flex items-center gap-3 relative">
                <span className="text-[13px] font-medium text-gray-600 w-8">Fill</span>
                ...
                <div
                  ...
                  className="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer shadow-sm relative color-picker-trigger"
                  style={{ backgroundColor: popupFillColor }}
                >
                  {popupFillColor === 'none' && <div className="absolute inset-0 bg-white flex items-center justify-center overflow-hidden"><div className="w-[140%] h-[1px] bg-red-500 rotate-45" /></div>}
                </div>
                <div className="flex-1 flex items-center border border-gray-300 rounded-xl px-3 py-2 bg-white focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition-all">
                  <input
                    type="text"
                    ...
                    className="w-full bg-transparent text-[13px] font-medium text-gray-700 outline-none uppercase"
                  />
                  <span
                    className="text-[12px] font-medium text-gray-400 ml-2 cursor-ew-resize select-none hover:text-indigo-600 transition-colors"
                    onMouseDown={(e) => {
                      e.preventDefault();
                      ...
                      const handleMove = (moveEvent) => {
                        const diff = Math.round((moveEvent.clientX - startX) / 2); // Divide by 2 for finer control
              ...
              {/* Stroke Row */}
              <div className="flex items-center gap-3 relative">
                <span className="text-[13px] font-medium text-gray-600 w-8">Stroke</span>
                <span className="text-[13px] font-medium text-gray-600">:</span>
                <div
                  onClick={() => { setShowStrokePicker(!showStrokePicker); setShowFillPicker(false); }}
                  className="w-10 h-10 rounded-lg border border-gray-300 cursor-pointer shadow-sm relative overflow-hidden flex items-center justify-center color-picker-trigger"
                  style={{ backgroundColor: popupStrokeColor === 'none' ? 'transparent' : popupStrokeColor }}
                >
                  {popupStrokeColor === 'none' && <div className="absolute inset-0 bg-white"><div className="w-[140%] h-[1.5px] bg-red-500 rotate-[-45deg] opacity-80 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" /></div>}
                </div>
                <div className="flex-1 flex items-center border border-gray-300 rounded-xl px-3 py-2 bg-white focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition-all">
                  <input
                    type="text"
              ...
              <div className="flex items-center gap-3">
                <div
                  className="p-2 rounded-lg hover:bg-indigo-50 transition-colors cursor-pointer group dashed-selector-trigger"
                  onClick={() => setShowDashedSettings(!showDashedSettings)}
                >
                  <SlidersHorizontal size={18} className={`transition-colors ${showDashedSettings ? 'text-indigo-600' : 'text-gray-400 group-hover:text-indigo-600'}`} />
                </div>
                <div className="flex-1 relative">
                  <select
                    ...
                    className="w-full h-10 bg-white border border-gray-300 rounded-xl px-3 text-[13px] font-medium text-gray-700 appearance-none outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                  >
                    <option value="dashed">Dashed</option>
                    <option value="solid">Solid</option>
                  </select>
                  <ChevronUp size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 rotate-180" />
                </div>
                <div
                  className="w-16 h-10 border border-gray-300 rounded-xl flex items-center px-2 gap-2 bg-white hover:border-indigo-400 transition-all cursor-ew-resize select-none shadow-sm"
                  onMouseDown={(e) => {
                ...
                >
                  <Icon icon="material-symbols:line-weight" className="text-gray-400 flex-shrink-0" size={18} />
                  <input
                    type="number"
                    ...
                    className="w-full bg-transparent text-[13px] font-medium text-gray-700 outline-none text-center cursor-text [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
                </div>
              </div>
              ...
              {showDashedSettings && ReactDOM.createPortal(
                <div
                  ref={dashedRef}
                  className="fixed right-[340px] top-[40%] translate-y-0 w-[280px] bg-white border border-gray-100 rounded-[28px] shadow-[0_20px_50px_rgba(0,0,0,0.2)] z-[100002] overflow-hidden animate-in fade-in zoom-in-95 duration-200"
                  style={{ transform: 'translateX(-20px)' }}
                >
                  <div className="p-6 space-y-5">
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-[15px] text-[#2D3748]">Dashed</span>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-[14px] font-medium text-[#4A5568]">Position :</span>
                        <div className="relative" ref={strokePositionRef}>
                          <div
                            onClick={() => setShowStrokePositionDropdown(!showStrokePositionDropdown)}
                            className="w-[120px] h-10 px-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between cursor-pointer hover:border-indigo-300 transition-all shadow-sm"
                          >
                            <span className="text-[13px] font-bold text-[#2D3748] capitalize">{popupStrokePosition}</span>
                            <ChevronUp size={14} className={`text-gray-400 transition-transform ${showStrokePositionDropdown ? '' : 'rotate-180'}`} />
                          </div>
                          {showStrokePositionDropdown && (
                            <div className="absolute top-full left-0 w-full mt-1 bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-1 overflow-hidden">
                              {['outside', 'center', 'inside'].map(pos => (
                                <div
                                  ...
                                  className={`px-4 py-2 text-[13px] hover:bg-indigo-50 hover:text-indigo-600 cursor-pointer capitalize font-bold transition-colors ${popupStrokePosition === pos ? 'text-indigo-600 bg-indigo-50' : 'text-[#4A5568]'}`}
                                >
                                  {pos}
                                </div>
                              ))}
                      ...
                      {/* Length */}
                      <div className="flex items-center justify-between">
                        <span className="text-[14px] font-medium text-[#4A5568]">Length :</span>
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center hover:text-indigo-600 transition-colors text-gray-400"
                          ><ChevronLeft size={16} /></button>
                          <div className="w-[50px] h-10 border border-gray-200 rounded-xl flex items-center justify-center font-bold text-[14px] text-[#2D3748] bg-white shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)]">
                      ...
                      {/* Gap */}
                      <div className="flex items-center justify-between">
                        <span className="text-[14px] font-medium text-[#4A5568]">Gap :</span>
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => {
                      ...
                      {/* Round Corners */}
                      <div className="flex items-center justify-between">
                        <span className="text-[14px] font-medium text-[#4A5568]">Round Corners :</span>
                        <div
                          onClick={() => {
                          ...
                          className={`w-11 h-6 rounded-full p-1 cursor-pointer transition-colors duration-200 ${popupStrokeRoundCorners ? 'bg-indigo-600' : 'bg-gray-200 shadow-inner'}`}
                        >
                          <div className={`w-4 h-4 bg-white rounded-full transition-transform duration-200 shadow-sm ${popupStrokeRoundCorners ? 'translate-x-5' : 'translate-x-0'}`}></div>
                        </div>
                      </div>
                    </div>
                  </div>
